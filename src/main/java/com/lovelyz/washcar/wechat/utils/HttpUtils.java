...
public class HttpUtils {
		...
		try {
			...
			if (url != null) {
				...
				if (data != null && !data.isEmpty()) {
					...
					output.write(data.getBytes(WechatConfig.CHARACTER_ENCODING));
					output.flush();
					output.close();
				}
	 ...
	 * @return 是否下载成功 Reuslt.success==true表示下载成功
	 */
	...
	public static InputStream toInputStream(String inputStr) throws IOException {
		...
		return IOUtils.toInputStream(inputStr, WechatConfig.CHARACTER_ENCODING); // IOUtils.toInputStream(inputStr);//不写好像是默认UTF-8
	}
